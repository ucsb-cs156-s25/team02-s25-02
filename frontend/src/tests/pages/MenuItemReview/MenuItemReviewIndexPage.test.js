...
describe("MenuItemReviewIndexPage tests", () => {
  ...
  test("renders multiple menu item reviews correctly for regular user", async () => {
    setupUserOnly();
    axiosMock
      .onGet("/api/menuitemreview/all")
      .reply(200, menuItemReviewFixtures.menuItemReviews);
    ...
    // Check for specific content in the reviews
    expect(screen.getAllByText("shiyuanwang@ucsb.edu").length).toBeGreaterThan(
      0,
    );
    expect(
